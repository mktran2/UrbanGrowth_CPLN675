...
aggregatedRasters <- aggregatedRasters %>% 
  mutate(developed2011 = ifelse(dev > 0.3,1,0), # made a difference between what was considered development in 2011 and 2021 
...
```
# Model 4 shows the most promise. # option 2 
```{r, warning = FALSE, message = FALSE}
testSetProbs <- 
  data.frame(class = datTest$development_change2,
...
dat_infill %>%
  ...
  scale_colour_manual(values = palette5,
                    ...
                    name="Quintile\nBreaks") +
  geom_sf(data=pop_2011, fill=NA, colour="black", size=1) +
  labs(title= "Development Demand in 2031: Predicted Probabilities by Census Tract") +
  mapTheme # DO WE WANT TO CHANGE THIS TO COUNTY?
```
