...
```{r, warning = FALSE, message = FALSE}
spatialCV_counties <-
  spatialCV(dat,"NAME","development_change2", Model4) %>%
  mutate(predClass = as.factor(ifelse(probs >= 0.17 ,1,0))) 
spatialCV_counties$predclass <- as.factor(spatialCV_counties$predclass)
...
dat_infill %>%
  ...
  scale_colour_manual(values = palette5,
                    ...
                    name="Quintile\nBreaks") +
   geom_sf(data=studyAreaCounties, fill=NA, colour="black", size=5) +
...
# 6.3 Creating sensitive regions
```{r, warning = FALSE, message = FALSE, fig.height = 6, fig.width= 6}
sensitiveRegions <- 
