...
```
# 1. Introduction
For over a decade, the Austin-Round Rock-San Marcos Metropolitan Statistical Area (MSA) has been the fastest growing metro area in the nation. As climate change threatens Greater Austin and the rest of Central Texas with longer, hotter, and drier summers, it is imperative that Austin regional planners prioritize the region’s natural resources while ensuring that development occurs safely. We created a model to do just that.

To create the model, we assumed development is attracted to existing development and accessibility. Based on this assumption and identifying sensitive environmental areas like wetlands and forests, we mapped where development should be allocated.

We modeled two scenarios of how development may occur in 2031: 1) business-as-usual (BAU) 2) Project Connect. Using historical development patterns from Scenario 1, we created a model to forecast where development may occur by 2031. The following sections outline how we created our model through Scenario 1 and how we utilized the model in Scenario 2. 

Ultimately, we found that **Williamson County** will experience the highest mean development demand by 2031 as well as a considerable population increase. Planning efforts should prioritize new development in Williamson County. We recommend that planning for new development in Williamson County should prioritize where residents will most need and use development, while taking care to protect the county’s wetlands and forests (see our Development Allocation Map)
...
# 2. Data Wrangling and Feaature Engineering 
The business-as-usual (BAU) scenario serves as the basis for our model. Using development and land cover data from 2011 and 2021, we created a model that predicts where development demand may occur in 2031. The following code create functions and the fishnet needed to build our model. 
```{r}
...
The BAU scenario operates on the assumption that development is attracted to existing development (Steif, 2019). In other words, where development is already present, more development will follow. Additionally, the BAU scenario assumes that development demand is predicated on accessibility—the more accessible an area is, whether that be due to transit networks, its proximity to other development, or population density, the higher the demand for more development (Steif, 2019). 

Based on these assumptions alongside features of the natural environment, we came up with multiple factors that would most likely affect development demand: a) distance to highways, b) distance to existing development, c) population change, d) wetlands, e) forests, f) farms, and g) undeveloped land. 
...
```
Mapping development in 2011 and 2021, we were able to track development where and to what degree an area was developed.
```{r}
...
To build our model, we needed to test whether a factor was statistically significant to some degree of change in development. By mapping development in 2011 to 2021 and our factors of interest, we determined whether a factor was significantly associated with a certain change in development. For example, areas that experienced population change, especially in 2021, tended to result in more new development, making population change a notable indicator. Additionally, we tracked what kinds of land uses were being developed between 2011 and 2021. 

The following bar plots illustrate whether there is a significant statistical difference between areas that did or did not change based on a certain variable. For example, take the “lagDevelopment” variable which asks, based on an area’s proximity to existing development, how many areas experienced new development or no change? The bar plot shows that based on the variable, distance to existing development did not result in lots of new development. However, this could be because areas may already be highly developed. 
...
dat %>%
  ...
  plotTheme
```
The table below illustrates how much of a certain land use was developed. For instance, about 0.49% of regional forests were developed between 2011 to 2021. 
```{r, warning = FALSE, message = FALSE}
dat %>%
...
# 4. Predicting 2021 
## 4.2. Modeling
Attempting to project 2021 development based on 2011 data, we tested various combinations of our factors and validated our models on actual 2021 development data. We chose Model 6 which resulted in the most accurate predictions and included all of our factors of interest. Having validated our model with recorded 2021 development data, we predicted 2031 development demand by county. 
```{r, warning = FALSE, message = FALSE}
set.seed(3456)
trainIndex <- 
...
nrow(dat)
```
The table illustrates how much of a certain land use was developed. For instance, about 0.49% of regional forests were developed between 2011 to 2021.
```{r, warning = FALSE, message = FALSE}
...
The histogram illustrates the distribution of developed or no change predictions. In order to really verify our model, we need to test the model against a smaller development classification threshold 
```{r, warning = FALSE, message = FALSE}
testSetProbs <- 
...
```
## 4.3. Accuracy
The confusion metrics measure a model’s sensitivity, specificity, and overall accuracy. Sensitivity refers to our model’s true positive predictions, that is, when change to development is predicted correctly. Specificity refers to our model’s false negatives, that is, when change to development is predicted incorrectly. In these maps, two development classification thresholds are implemented: 5 percent and 17 percent. In both our 5 percent and 17 percent thresholds, our model tends to correctly predict a high number of areas changed to “developed.” However, while our 5 percent threshold predicts both a high number of “developed” areas and under-predicts the number of “no change” areas, the 17 percent threshold is overall more accurate, because our data’s “no change” areas far outnumber areas where change was actually observed. Ultimately, the 17 percent threshold offers a more accurate modeling of Greater Austin, which tends to already be highly developed. 
```{r, warning = FALSE, message = FALSE}
...
```
The map below shows development demand for the region, closely following the existing highway network. 
```{r, warning = FALSE, message = FALSE}
...
# 6 Comparing Predicted Development Deemand and Environmental Sensitivity
## 6.2. 2021 Land Cover Data
```{r, warning = FALSE, message = FALSE, fig.height = 6, fig.width= 6}
# land cover for 2021
lc_austin_21 <- crop(x = lc_austin_21, y = studyAreaCounties)
...
ggplot() +
  ...
  mapTheme # the dataset was too big to run
```

## 6.5. Summarize by County
The development demand metrics provides supply and demand side analytics for all counties. Based on the table, Williamson County will experience the most mean development demand as well as a considerable population change. 
```{r, warning = FALSE, message = FALSE, fig.height = 6, fig.width= 6}
# summarize by county 
# We have to editing pop_2031 table to get "NAME" to match first # csm = tables only for county_specific_metrics
countypop_2031csm <- 
  data.frame(
   NAME = 
...
pop_2021csm$NAME <- str_extract(pop_2021csm$NAME,"\\b\\w+\\b")
...
# 7. Demand Allocation 
Based on our 2031 Development Demand map and our county-specific analytics, we observed Williamson County as the county that will experience the highest development demand alongside considerable population increase. We determined where development would best be allocated by identifying areas of high development demand and population increase while prohibiting development on sensitive environmental land like wetlands and forest. 
## 7.2. Demand Allocation: Williamson County
The two maps below show Williamson County: the map (left) illustrates where development is most suitable and where it is not based on sensitive environmental land like wetlands and forests, and the map (right) depicts which areas of the county may experience population change. Based on the development potential map and projected population map, development is suitable and most desirable in the dark blue areas.
...
williamson_landUse <- rbind(
  filter(williamson, forest21 == 1 | wetlands21 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(williamson, developed21 == 1) %>%
...
ggplot() +
  ...
  geom_point(data=williamson_landUse, aes(x=xyC(williamson_landUse)[,1], 
                                        ...
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="Williamson")), size=2) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(williamson,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Development Potential, 2031: Williamson") + mapTheme +
...
ggplot() +
  ...
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Projected Population, 2031: Williamson") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)

```
## 7.3. County-Specific Demand Allocation Maps 
The map below shows a demand allocation map for Travis County.
```{r, warning = FALSE, message = FALSE, fig.height= 8, fig.width= 11}
travis  <-
  dat2 %>%
    mutate(Development_Demand = predict(Model6, dat2, type="response")) %>%
...
travis_landUse <- rbind(
  filter(travis, forest21 == 1 | wetlands21 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(travis, developed21 == 1) %>%
  dplyr::select() %>% mutate(Land_Use = "Developed"))
...
ggplot() +
  ...
  geom_point(data=travis_landUse, aes(x=xyC(travis_landUse)[,1], 
                                        ...
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="Travis")), size=2) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(travis,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Development Potential, 2031: Travis") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)),
...
ggplot() +
  ...
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Projected Population, 2031: Travis") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)
```
The map below shows a demand allocation map for Hays County.
```{r, warning = FALSE, message = FALSE, fig.height= 8, fig.width= 11}
hays  <-
  dat2 %>%
    mutate(Development_Demand = predict(Model6, dat2, type="response")) %>%
...
hays_landUse <- rbind(
  filter(hays, forest21 == 1 | wetlands21 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(hays, developed21 == 1) %>%
  dplyr::select() %>% mutate(Land_Use = "Developed"))
...
ggplot() +
  ...
  geom_point(data=hays_landUse, aes(x=xyC(hays_landUse)[,1], 
                                        ...
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="Hays")), size=2) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(hays,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Development Potential, 2031: Hays") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)),
...
ggplot() +
  ...
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Projected Population, 2031: Hays") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)
```
The map below shows a demand allocation map for Caldwell County.
```{r, warning = FALSE, message = FALSE, fig.height= 8, fig.width= 11}
caldwell  <-
  dat2 %>%
    mutate(Development_Demand = predict(Model6, dat2, type="response")) %>%
...
caldwell_landUse <- rbind(
  filter(caldwell, forest21 == 1 | wetlands21 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(caldwell, developed21 == 1) %>%
  dplyr::select() %>% mutate(Land_Use = "Developed"))

grid.arrange(
ggplot() +
  geom_sf(data=caldwell, aes(fill=factor(ntile(Development_Demand,5))), colour=NA) +
  geom_point(data=caldwell_landUse, aes(x=xyC(caldwell_landUse)[,1], 
                                        y=xyC(caldwell_landUse)[,2], colour=Land_Use),
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="caldwell")), size=2) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(caldwell,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Development Potential, 2031: Caldwell") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)),

ggplot() +
  geom_sf(data=caldwell, aes(fill=factor(ntile(pop_Change,5))), colour=NA) +
  geom_point(data=caldwell_landUse, aes(x=xyC(caldwell_landUse)[,1], 
                                        y=xyC(caldwell_landUse)[,2], colour=Land_Use),
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="caldwell")), size=2) +
  scale_fill_manual(values = palette5, name="Population\nChange",
                    labels=substr(quintileBreaks(caldwell,"pop_Change"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Projected Population, 2031:Caldwell") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)
```
The map below shows a demand allocation map for Bastrop County.
```{r, warning = FALSE, message = FALSE, fig.height= 8, fig.width= 11}
bastrop  <-
  dat2 %>%
    mutate(Development_Demand = predict(Model6, dat2, type="response")) %>%
...
bastrop_landUse <- rbind(
  filter(bastrop, forest21 == 1 | wetlands21 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(bastrop, developed21 == 1) %>%
  dplyr::select() %>% mutate(Land_Use = "Developed"))

grid.arrange(
ggplot() +
  geom_sf(data=bastrop, aes(fill=factor(ntile(Development_Demand,5))), colour=NA) +
  geom_point(data=bastrop_landUse, aes(x=xyC(bastrop_landUse)[,1], 
                                        y=xyC(bastrop_landUse)[,2], colour=Land_Use),
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="bastrop")), size=2) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(bastrop,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Development Potential, 2031: Bastrop") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)),

ggplot() +
  geom_sf(data=bastrop, aes(fill=factor(ntile(pop_Change,5))), colour=NA) +
  geom_point(data=bastrop_landUse, aes(x=xyC(bastrop_landUse)[,1], 
                                        y=xyC(bastrop_landUse)[,2], colour=Land_Use),
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(MSAHighways,filter(studyAreaCounties, NAME=="bastrop")), size=2) +
  scale_fill_manual(values = palette5, name="Population\nChange",
                    labels=substr(quintileBreaks(bastrop,"pop_Change"),1,5)) +
  scale_colour_manual(values = c("black","red")) + 
  labs(title = "Projected Population, 2031: Bastrop") + mapTheme +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)
```

# Scenario 2: Project Connect
```{r, warning = FALSE, message = FALSE}
library(leaflet)
library(leafem)
